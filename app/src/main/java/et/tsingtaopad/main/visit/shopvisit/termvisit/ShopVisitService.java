...
import et.tsingtaopad.core.util.dbtutil.ImageUtil;
...
/**
 * Created by yangwenmin on 2017/12/28.
 */
public class ShopVisitService {
     ...
     */
    public void confirmUpload(String visitId, String termId, String visitEndDate, String uploadFlag) {
        // 事务控制
        ...
        try {
            ...
            if (ConstValues.FLAG_1.equals(uploadFlag)) {// 恒成立
    ...
    // 复制各个临时表
    public String toCopyData(MstTermListMStc termStc) {
        // 从拜访主表中获取最后一次拜访数据
        ...
        try {
            ...
            //MstTerminalinfoMTemp termTemp = findTermTempById(termStc.getTerminalkey());
    ...
    private void createMstCollectionexerecordInfoTemp(Dao<MstCollectionexerecordInfo, String> collectionDao,
                                                      ...
                                                      String prevVisitId,MstTermListMStc termStc,MstVisitMTemp visitMTemp) {
        try {
    ...
    /**
     * 从临时表中获取 获取分项采集部分的产品指标对应的采集项目数据 mst_vistproduct_info_temp -- mst_collectionexerecord_info_temp
     *
     * @param visitId   拜访主键
     ...
     */
    public void saveCheckIndex(String visitId, String termId, List<ProIndex> calculateLst, List<ProItem> proItemLst, List<CheckIndexCalculateStc> noProIndexLst) {
        AndroidDatabaseConnection connection = null;
        try {
            DatabaseHelper helper = DatabaseHelper.getHelper(context);
    ...
    // 保存照片记录到数据库
    public void savePicData(CameraInfoStc cameraDataStc, String picname, String imagefileString, String termId, String visitKey) {
    ...
    // 添加水印
    public Bitmap addWaterBitmap(Bitmap bitmap, String time, String username, String termname, String pictypename) {
        // 水印
        Bitmap waterBitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.bg_picc);
        waterBitmap = FunUtil.zoomImg(waterBitmap, ViewUtil.dip2px(context, 640), ViewUtil.dip2px(context, 30));
        bitmap = ImageUtil.createWaterMaskLeftBottom(context, bitmap, waterBitmap, 0, 0);
        // (上下文,bm,文字内容,字体大小,字体颜色,距左距离,距下距离)
        bitmap = ImageUtil.drawTextToLeftBottom(context, bitmap, "日期: " + time, 10, Color.WHITE, ViewUtil.dip2px(context, 5), ViewUtil.dip2px(context, 14));
        bitmap = ImageUtil.drawTextToLeftBottom(context, bitmap, "业代: " + username, 10, Color.WHITE, ViewUtil.px2dip(context, 288), ViewUtil.dip2px(context, 14));
        bitmap = ImageUtil.drawTextToLeftBottom(context, bitmap, "终端: " + termname, 10, Color.WHITE, ViewUtil.dip2px(context, 5), ViewUtil.dip2px(context, 3));
        bitmap = ImageUtil.drawTextToLeftBottom(context, bitmap, "照片: " + pictypename, 10, Color.WHITE, ViewUtil.px2dip(context, 288), ViewUtil.dip2px(context, 3));
