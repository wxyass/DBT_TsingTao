...
public class TermListAdapter extends BaseAdapter implements OnClickListener {
    ...
    private List<MstTermListMStc> dataLst;// 需要展示的终端集合 (搜索框有值:对应模糊搜索出来的终端,没值:对应所有终端)
    private List<MstTermListMStc> seqTermList;// 该路线下所有终端
    private RelativeLayout confirmBt;
    private String termId;
    private int selectItem = -1;
    private boolean isUpdate;//是否处于修改状态

    public TermListAdapter(Context context, List<MstTermListMStc> seqTermList, List<MstTermListMStc> termialLst, RelativeLayout confirmBt, String termId) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(context).inflate(R.layout.shopvisit_terminal_lvitem_t, null);
            holder.terminalSequenceEt = (EditText) convertView.findViewById(R.id.term_item_et_sequence);// 排序序号
            holder.terminalNameTv = (TextView) convertView.findViewById(R.id.term_item_tv_name);// 终端名称
            holder.visitDateTv = (TextView) convertView.findViewById(R.id.term_item_tv_visitdate);// 拜访时间
            holder.terminalRb = (RadioButton) convertView.findViewById(R.id.term_item_rb);// RadioButton
            holder.terminalTypeTv = (TextView) convertView.findViewById(R.id.term_item_tv_type);// 终端渠道类型
            holder.updateIv = (ImageView) convertView.findViewById(R.id.term_item_iv_update);// 上传成功标识
            holder.mineIv = (ImageView) convertView.findViewById(R.id.term_item_iv_mime);// 我品
            holder.mineProtocolIv = (ImageView) convertView.findViewById(R.id.term_item_iv_mineprotocol);// 我品协议店
            ...
            holder.itermLayout = (LinearLayout) convertView.findViewById(R.id.itrm_item_ll);// 整个条目view
            holder.itemCoverV = convertView.findViewById(R.id.term_item_v_cover);// 失效终端底色
            // 排序序号监听
        ...
        if (!CheckUtil.isBlankOrNull(item.getVisitTime())) {// 当天有过拜访
        ...
        if (ConstValues.FLAG_1.equals(item.getSyncFlag())) {// 当天最后一次拜访 SyncFlag  1:已成功上传  0:未成功上传 null:当天该终端没拜访
        ...
        if (position == selectItem) {
            ...
            // 当天已提交过的(不管是否上传成功)  设为红色  // 当天最后一次拜访 UploadFlag 0:未确定上传   1:已确定上传 null:当天该终端没拜访
    ...
    private void saveEditValue(String str, int position) {
        MstTermListMStc termStc = dataLst.get(position);
        if (isUpdate && !str.equals(termStc.getSequence())) {
            resetSeq(termStc.getTerminalkey(), str);
        }
        termStc.setSequence(str);
    }
