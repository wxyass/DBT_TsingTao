...
public class TermListService {
    ...
    /**
     * √ 相应的数据列表数据(获取某路线下的所有终端的所有信息 包含店及离店时间)
     *
     * @param //lineId 线路表主键
     * @return
     */
    public List<MstTermListMStc> queryTerminal(List<String> lineKeys) {
        ...
        try {
            ...
            // 当天时间
            String currentday = DateUtil.formatDate(new Date(), "yyyyMMdd");

            // ↓--- visitTimeMap: 获取各终端当天拜访的进店及离店时间, key:终端主键、value:拜访时间
            Map<String, String> visitTimeMap = new HashMap<String, String>();
            for (int i = 0; i < lineKeys.size(); i++) {

                // 获取某线路下的各终端当天的所有拜访进店及离店时间 (路线key -> 该路线下的所有终端 -> 终端对应的当天所有拜访  )
                // 只取出匹配的数据(一家终端当天拜访了3次则取3条, 另一家终端一次也没拜访取0条)
                List<MstTermListMStc> termlst = dao.queryTermVistTimeByLoginDate(helper, lineKeys.get(i), currentday);
                StringBuffer buffer;
                for (MstTermListMStc item : termlst) {
                    buffer = new StringBuffer();
                    if (item.getVisitDate() != null && item.getVisitDate().length() >= 14) { // -> 15:08:04
                    ...
                    if (item.getEndDate() != null && item.getEndDate().length() >= 14) { // -> 15:23:11
                ...
                // 获取路线中下各终端信息 ,涉及到v_visit_m_newest
                ...
                for (MstTermListMStc item : termlst) {
                    if (visitTimeMap.containsKey(item.getTerminalkey())) {
                        ...
                        visitTime = visitTime.substring(0, visitTime.length() - 2);// 去掉进店及离店时间最后的、
    ...
    /**
     * 废弃 获取相应的数据列表数据 今天已拜访终端(获取某路线下的所有终端的所有信息 包含店及离店时间)
     * @param //lineId 线路表主键
     * @return
     */
    ...
    public List<MstTermListMStc> queryTerminal3(List<String> lineKeys) {
        ...
        try {
            ...
            for (int i = 0; i < lineKeys.size(); i++) {
                ...
                // 获取路线中下各终端信息 ,涉及到v_visit_m_newest 废弃整这个方法
