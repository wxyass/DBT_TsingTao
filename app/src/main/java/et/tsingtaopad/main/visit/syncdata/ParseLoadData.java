...
public class ParseLoadData {
	private final String TAG = "ParseLoadData";
	...
	public ParseLoadData(Context context, Handler handler) {
	 ...
	 * 更新问题反馈
	 * 
	 * @param //mstQuestionsanswersInfos
	 */
		...
		try {
			...
			for (String key : responseHashMap.keySet()) {
				...
				if (key.toUpperCase().indexOf("MST_VISIT_M_ZIP") != -1) {
					...
					if (zipFlag) {
						json = ZipUtil.readFile(zipPath + key + ".txt");
						//FileUtil.writeTxt(json,FileUtil.getSDPath()+"/zipPath_MST_AGENCYSUPPLY_INFO_ZIP2.txt");
					} else {
						json = (String) responseHashMap.get(key);
					}
					...
					//count++;
					//FileUtil.writeTxt(json,FileUtil.getSDPath()+"/MST_AGENCYSUPPLY_INFO_ZIPww.txt");
					...
					//FileUtil.writeTxt(json, FileUtil.getSDPath()+"/MST_VISTPRODUCT_INFO_ZIP08152.txt");
					...
					//FileUtil.writeTxt(json, FileUtil.getSDPath()+"/MST_CHECKEXERECORD_INFO_ZIPw2w.txt");
	 ...
	 * 万能铺货率终端类型
	 * 
	 * @param //MST_POWERFULTERMINAL_INFO
	 */
