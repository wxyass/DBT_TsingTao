...
import et.tsingtaopad.main.operation.orders.vhtable.VHBaseAdapter;
...
public class VHTableAdapter implements VHBaseAdapter {
    ...
    public View getTitleView(int columnPosition, ViewGroup parent) {
        ...
        if(0==columnPosition){
            ...
            //tv_item.setPadding(64, 20, 64, 20);// 标题第一列内边距
            tv_item.setPadding(64, 10, 64, 10);// 标题第一列内边距
        }else {
            //tv_item.setPadding(100, 40, 100, 40);
            //tv_item.setPadding(88, 20, 88, 20);// 标题其它列内边距(除第一列)
            tv_item.setPadding(88, 10, 88, 10);// 标题其它列内边距(除第一列)
        }
        tv_item.setText(titleData.get(columnPosition));
        tv_item.setGravity(Gravity.CENTER);
        //tv_item.setTextSize(19);
        tv_item.setTextColor(context.getResources().getColor(R.color.darktitle));
        return tv_item;
    }
