...
public class PromotionService {
	...
	public void search(final MstPromotionsM prom, final String[] lineIds,
                       final String[] termLevels, final String searchDate, AlertDialog dialog) {
	    if ("-1".equals(prom.getPromotkey())) {
	        ...
	        // 拜访数据缓存最大日期
	        String visitED = "";
	        MstSynckvM visitKv = GlobalValues.kvMap.get("MST_VISIT_M");
	        ...
	        } else if (!CheckUtil.isBlankOrNull(visitED) 
	                && visitED.compareTo(prom.getStartdate()) <= 0) {
	            new Thread() {
                    @Override
                    public void run() {
                        super.run();
