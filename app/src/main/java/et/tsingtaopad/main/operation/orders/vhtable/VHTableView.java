package et.tsingtaopad.main.operation.orders.vhtable;
...
public class VHTableView extends LinearLayout implements HListViewScrollView.ScrollChangedListener {
	...
	private void initContentList(VHBaseAdapter conentAdapter) {
		...
		listView.setVerticalScrollBarEnabled(false);// 取消滚动条
	...
	private ViewHolder updateViews(VHBaseAdapter conentAdapter, ViewHolder viewHolder, LinearLayout ll_firstcolumn,
			LinearLayout ll_datagroup, int row) {
	...
	private void updateUI(VHBaseAdapter conentAdapter, LinearLayout ll_firstcolumn, LinearLayout ll_datagroup,
			View[] views, int maxHeight) {
