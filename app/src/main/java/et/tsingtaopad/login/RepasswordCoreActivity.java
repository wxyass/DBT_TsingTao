 ...
 * 登录
 */
public class RepasswordCoreActivity extends BaseCoreActivity implements View.OnClickListener{
            ...
            switch (msg.what) {
                ...
                case ConstValues.WAIT1:
                    ...
                    if (isSuccess) {
                        ...
                        if("a1234567".equals(userPwd)){// 1 原始密码-->需修改
                            // 跳到修改密码界面
                            Toast.makeText(getBaseContext(), "请先修改原始密码", Toast.LENGTH_SHORT).show();
                            Intent intent = new Intent(RepasswordCoreActivity.this, RepasswordCoreActivity.class);
                            startActivity(intent);
                            RepasswordCoreActivity.this.finish();
                        }else if(userPwd.length()<8){// 2 密码不是8位-->需修改-->跳到修改密码界面

                        }
                        else if(!userPwd.matches("^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,16}$")){// 3 密码不是数字加英文组合-->需修改-->跳到修改密码界面

                        }
                        else if(Integer.parseInt(isrepassword)<= 0){
